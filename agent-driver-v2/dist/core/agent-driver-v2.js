...
export class AgentDriverV2 {
    ...
    async collectMessages() {
        ...
        for (const source of this.sources) {
            try {
                ...
                this.logger.debug(`Collected ${messages.length} messages from ${source.name}`);
            }
            catch (error) {
                this.logger.error(`Failed to collect messages from ${source.name}:`, error);
            }
        }
        // 按时间戳排序
                ...
                if (typeof message.content === 'string') {
                    ...
                    try {
                        ...
                        if (Array.isArray(parsed) && parsed.every((part) => part && typeof part === 'object' && typeof part.type === 'string')) {
                            message = { ...message, content: parsed };
                        }
                    }
                    catch {
        ...
        const getToolResultIds = (message) => {
            if (message.role !== 'tool' || !Array.isArray(message.content)) {
                return [];
            }
            return message.content
        ...
        const usedToolMessages = new Set();
        const reordered = [];
    ...
    async collectTools() {
        ...
        for (const source of this.sources) {
            try {
                ...
                this.logger.debug(`Collected ${Object.keys(tools).length} tools from ${source.name}`);
            }
            catch (error) {
                this.logger.error(`Failed to collect tools from ${source.name}:`, error);
            }
        }
    ...
    async executeToolCalls(toolCalls) {
        ...
        for (const toolCall of toolCalls) {
            ...
            if (source) {
                try {
                    const result = await source.executeTool(toolCall.toolName, toolCall.args, toolCall.toolCallId);
                    if (result) {
                        results.push(result);
                        this.logger.debug(`Tool ${toolCall.toolName} executed by ${source.name}`);
                    }
                    else {
                        // Source 返回 undefined，Tool 不属于它
                        this.logger.warn(`Tool ${toolCall.toolName} not found in ${source.name}`);
                        results.push({
                ...
                // 没有找到对应的 source
                this.logger.warn(`No DrivenSource found for tool: ${toolCall.toolName}`);
                results.push({
                    toolCallId: toolCall.toolCallId,
                    toolName: toolCall.toolName,
                    error: {
    ...
    async updateToolMapping() {
        ...
        for (const source of this.sources) {
            try {
                ...
                this.logger.error(`Failed to update tool mapping from ${source.name}:`, error);
            }
        }
        this.logger.debug(`Tool mapping updated: ${this.toolToSourceMap.size} tools`);
    }
    /**
     * 设置状态
     */
     ...
     */
    buildInputSignature(messages, tools) {
